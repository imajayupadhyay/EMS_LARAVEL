...
class LeaveApplicationController extends Controller
{
    public function index(Request $request)
    {
        $applications = LeaveApplication::with(['employee', 'leaveType'])
            ...
            ->when($request->employee, fn($q) => 
                $q->whereHas('employee', fn($q2) => 
                    $q2->whereRaw("CONCAT(first_name, ' ', last_name) LIKE ?", ["%{$request->employee}%"])
                )
            )
            ->when($request->leave_type, fn($q) => 
                $q->whereHas('leaveType', fn($q2) => 
                    $q2->where('name', $request->leave_type)
                )
            )
    ...
    public function update(Request $request, LeaveApplication $leaveApplication)
    {
        ...
        if ($request->status === 'approved') {
            ...
            $assignment = LeaveAssignment::where('employee_id', $leaveApplication->employee_id)
