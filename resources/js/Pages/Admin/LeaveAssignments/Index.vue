...
const edit = (assignment) => {
  form.user_id = assignment.user.id
...
const deleteAssignment = (assignment) => {
  if (confirm(`Delete leave assignment for ${assignment.user.name}?`)) {
...
<template>
  <AdminLayout>
    ...
    <div class="max-w-5xl mx-auto p-4">
      <h1 class="text-2xl font-bold text-orange-600 mb-4">📝 Leave Assignments</h1>

      <div v-if="flash?.success" class="bg-green-100 text-green-700 px-4 py-2 rounded mb-4 border border-green-300 shadow">
        {{ flash.success }}
      </div>
      ...
      <div class="bg-white rounded-lg shadow p-4 mb-6 border border-gray-200">
        ...
        <form @submit.prevent="submit" class="space-y-3">
          <div>
            <select v-model="form.user_id" class="border rounded w-full px-3 py-2">
          ...
          <div>
            <input type="number" v-model="form.total_assigned" placeholder="Total Assigned" class="border rounded w-full px-3 py-2" />
            <div v-if="form.errors.total_assigned" class="text-red-600 text-sm">{{ form.errors.total_assigned }}</div>
          </div>
          <div>
            <input type="number" v-model="form.balance" placeholder="Balance" class="border rounded w-full px-3 py-2" />
      ...
      <div class="bg-white rounded-lg shadow border border-gray-200">
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="a in assignments.data" :key="a.id" class="border-t hover:bg-gray-50 transition">
              <td class="p-2">{{ a.user.name }}</td>
              <td class="p-2">{{ a.leave_type.name }}</td>
              <td class="p-2">{{ a.total_assigned }}</td>
        ...
        <div class="flex justify-between p-3">
          <button v-if="assignments.prev_page_url" @click="router.get(assignments.prev_page_url, {}, { preserveScroll: true })" class="bg-orange-500 text-white px-3 py-1 rounded shadow hover:bg-orange-600">Previous</button>
          <button v-if="assignments.next_page_url" @click="router.get(assignments.next_page_url, {}, { preserveScroll: true })" class="bg-orange-500 text-white px-3 py-1 rounded shadow hover:bg-orange-600">Next</button>
        </div>
      </div>
    </div>
  </AdminLayout>
