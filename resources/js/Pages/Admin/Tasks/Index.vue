...
<template>
  <AdminLayout>
    <Head title="View Employee Tasks" />
    <div class="max-w-7xl mx-auto p-4">
      ...
      <div class="bg-orange-50 border border-orange-200 rounded-lg p-4 shadow mb-6 animate-fade-in">
        <div class="flex flex-col md:flex-row gap-3">
          <div class="flex items-center gap-2">
            ...
            <input type="date" v-model="selectedDate" @change="applyFilters" class="border rounded px-2 py-1 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
          </div>
          <div class="flex items-center gap-2">
            <label class="font-medium">Employee:</label>
            <input type="text" v-model="employeeName" @input="applyFilters" placeholder="Search by name" class="border rounded px-2 py-1 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
          </div>
        </div>
      </div>
      ...
      <div v-if="props.tasks.data.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="font-semibold text-lg text-gray-800 mb-1">{{ task.user.name }}</div>
          <div class="truncate text-gray-600 text-sm" v-html="task.task_content"></div>
        </div>
      </div>
      <p v-else class="text-gray-500 animate-fade-in">No tasks found for selected filters.</p>

      <!-- Pagination -->
      <div class="mt-6 flex justify-center gap-4 animate-fade-in">
        <button
          v-if="props.tasks.prev_page_url"
          @click="router.get(props.tasks.prev_page_url, {}, { preserveScroll: true })"
          class="px-4 py-2 bg-orange-500 text-white rounded shadow hover:bg-orange-600"
        >
          Previous
        </button>
        <button
          v-if="props.tasks.next_page_url"
          @click="router.get(props.tasks.next_page_url, {}, { preserveScroll: true })"
          class="px-4 py-2 bg-orange-500 text-white rounded shadow hover:bg-orange-600"
        >
          Next
        </button>
...
<transition name="fade">
  ...
  >
    <div class="bg-white rounded-lg max-w-lg w-full p-6 shadow-2xl relative transform animate-scale-in">
     ...
     <button
  class="absolute top-2 right-0 bg-gray-200 hover:bg-gray-300 text-gray-700 hover:text-black rounded-full w-8 h-8 flex items-center justify-center shadow"
  @click="closePopup"
>
  ✕
</button>
